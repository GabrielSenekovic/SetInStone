...
public class Pulka : MonoBehaviour
{
    ...
    public void SetState(PulkaState value, Movement movement)
    {
        ...
        switch(value)
        {
            ...
            case PulkaState.SITTING:
                ...
                movement.AddFlag(NiyoMovementState.SLIDE_REQUEST);
    ...
    {
        if (movement.HasFlag(NiyoMovementState.DISMOUNT_REQUEST) && movement.GetGrounded()) //* triggers when on the pulka on ground, so that you get pushed up
        {
            movement.GetBody().AddForce(new Vector2(0, movement.jumpForce), ForceMode2D.Impulse);
            movement.SetGrounded(false);
            movement.RemoveFlag(NiyoMovementState.DISMOUNT_REQUEST);
